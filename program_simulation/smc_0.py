...
def taus(s, beta, zeta, omega, t, q, e, p, l, g):
    ...
    taus = M(t,q,p,l) @ (((5 * np.identity(3, dtype=np.float64)) @ e[:,[1]]) + qd_dotdot + k_smc * np.sign(s)) + np.dot(C(t,q,p,l), q[:,[1]]) + G(t,q,l,p,g) + F(t,q)
    
    return taus

def tau_f(s, A, beta, zeta, omega, t, q, e, p, l, g):
    
    tau = np.empty((3,1), dtype=np.float64)
    
    K = 100 * np.identity(3, dtype=np.float64)

    tau = taus(s,beta,zeta,omega,t,q,e,p,l,g) + K @ s + y(A,W)
    #tau = y(A,W)
    #tau = taus(s,beta,zeta,omega) + K @ s
    #tau = taus(s,beta,zeta,omega,t,q,e,p,l,g)
...
def rungekutta_2(t, q, xold, p, l, g, odot, co, ro, W, beta, zeta):
    ...
    tau = tau_f(s,A,beta,zeta,omega,t,q,e,p,l,g)
...
while t < end:
    ...
    e7_data.append(taus(s,beta,zeta,omega,t,q,e,p,l,g)[0])
    e8_data.append(taus(s,beta,zeta,omega,t,q,e,p,l,g)[1])
    e9_data.append(taus(s,beta,zeta,omega,t,q,e,p,l,g)[2])
