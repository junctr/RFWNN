...
while t < end:
    ...
    e_k1,x_k1,k1_q,k1_odot,k1_co,k1_ro,k1_W,k1_beta,k1_zeta = rungekutta_2(t,q,xold[-T],p,l,g,odot,co,ro,W,beta,zeta)
    e_k2,x_k2,k2_q,k2_odot,k2_co,k2_ro,k2_W,k2_beta,k2_zeta = rungekutta_2(t+step/2,q+(step/2)*k1_q,xold[-T],p,l,g,odot+(step/2)*k1_odot,co+(step/2)*k1_co,ro+(step/2)*k1_ro,W+(step/2)*k1_W,beta+(step/2)*k1_beta,zeta+(step/2)*k1_zeta)
    e_k3,x_k3,k3_q,k3_odot,k3_co,k3_ro,k3_W,k3_beta,k3_zeta = rungekutta_2(t+step/2,q+(step/2)*k2_q,xold[-T],p,l,g,odot+(step/2)*k2_odot,co+(step/2)*k2_co,ro+(step/2)*k2_ro,W+(step/2)*k2_W,beta+(step/2)*k2_beta,zeta+(step/2)*k2_zeta)
    e_k4,x_k4,k4_q,k4_odot,k4_co,k4_ro,k4_W,k4_beta,k4_zeta = rungekutta_2(t+step,q+step*k3_q,xold[-T],p,l,g,odot+step*k3_odot,co+step*k3_co,ro+step*k3_ro,W+step*k3_W,beta+step*k3_beta,zeta+step*k3_zeta)
...
plt.title(f"s{n_seed}_m{alpha_lambda}_T{T}")
plt.show()

"""
with open("m.csv","w")as f:
    dataWriter = csv.writer(f)
    dataWriter.writerow(e2_data)

f.close()
"""

np.savetxt(f"s{n_seed}_m{alpha_lambda}_T{T}_e1.csv",e1_data)
np.savetxt(f"s{n_seed}_m{alpha_lambda}_T{T}_e2.csv",e2_data)
np.savetxt(f"s{n_seed}_m{alpha_lambda}_T{T}_e3.csv",e3_data)
